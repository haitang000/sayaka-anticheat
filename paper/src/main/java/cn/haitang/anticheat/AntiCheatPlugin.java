...
import cn.haitang.anticheat.check.world.BlockReachCheck;
import cn.haitang.anticheat.check.world.EntityReachCheck;
import cn.haitang.anticheat.check.world.FastBreakCheck;
import cn.haitang.anticheat.check.world.InteractAimCheck;
...
public final class AntiCheatPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        checks.add(new BlockReachCheck(this));
        checks.add(new EntityReachCheck(this));
        checks.add(new InteractAimCheck(this));
    ...
    private PersistentStore createStore() {
        ...
        String storage = runtimeConfig.getString("network.storage", "mariadb");
        if (!"mariadb".equalsIgnoreCase(storage) && !"mysql".equalsIgnoreCase(storage)) {
            getLogger().warning("network.storage 不是 mariadb/mysql，已回退到单服 data.yml");
            return new PersistentStore(this);
        }
        DatabaseConfig database = new DatabaseConfig(
                runtimeConfig.getString("network.database.jdbc-url", ""),
                runtimeConfig.getString("network.database.username", ""),
                runtimeConfig.getString("network.database.password", ""));
        return new NetworkPersistentStore(this, new JdbcNetworkStore(database),
                runtimeConfig.getString("network.server-id", "paper"));
    }
