...
public final class ConfigSnapshot {
     ...
     * strict+ = 最严格（buffer 全面收紧，弱检测全部升为 mitigate，宽限最短）
    ...
    private static void applyPreset(YamlConfiguration cfg, String preset) {
        ...
        if (!p.equals("strict-plus") && !p.equals("strict") && !p.equals("lenient")) {
            return; // balanced 或未知值：保持 bundled 默认
        }
        boolean strictPlus = p.equals("strict-plus");
        boolean strict = p.equals("strict") || strictPlus;
        Map<String, Object> overrides = new HashMap<>();
        // —— settings ——
        overrides.put("settings.join-grace-seconds", strictPlus ? 2 : strict ? 4 : 12);
        // —— speed ——
        overrides.put("checks.speed.buffer-to-flag", strictPlus ? 2.0 : strict ? 2.5 : 4.0);
        // —— flight ——
        overrides.put("checks.flight.buffer-to-flag", strictPlus ? 1.0 : strict ? 1.5 : 3.0);
        overrides.put("checks.flight.gravity-buffer-to-flag", strictPlus ? 2.0 : strict ? 3.0 : 5.0);
        // —— elytra ——
        overrides.put("checks.elytra.buffer-to-flag", strictPlus ? 1.0 : strict ? 1.5 : 3.0);
        // —— timer ——
        overrides.put("checks.timer.max-packets-per-second", strictPlus ? 20 : strict ? 22 : 28);
        // —— reach ——
        overrides.put("checks.reach.buffer-to-flag", strictPlus ? 1.5 : strict ? 2.0 : 3.5);
        // —— hitbox ——
        overrides.put("checks.hitbox.buffer-to-flag", strictPlus ? 3.0 : strict ? 3.5 : 5.5);
        // —— kill-aura ——
        overrides.put("checks.kill-aura.buffer-to-flag", strictPlus ? 1.5 : strict ? 2.0 : 3.5);
        // —— velocity ——
        overrides.put("checks.velocity.buffer-to-flag", strictPlus ? 2.0 : strict ? 2.5 : 4.5);
        // —— auto-totem ——
        overrides.put("checks.auto-totem.min-react-ms", strictPlus ? 60 : strict ? 75 : 120);
        // —— no-slow ——
        overrides.put("checks.no-slow.buffer-to-flag", strictPlus ? 2.0 : strict ? 2.5 : 4.5);
        // —— fast-break ——
        overrides.put("checks.fast-break.lenience", strictPlus ? 0.70 : strict ? 0.65 : 0.50);
        // —— scaffold ——
        overrides.put("checks.scaffold.buffer-to-flag", strictPlus ? 2.0 : strict ? 2.5 : 4.5);
        // —— anti-spam ——
        overrides.put("checks.anti-spam.max-messages", strictPlus ? 4 : strict ? 5 : 8);
        // —— 弱检测 enforcement 升级（strict / strict+ 才改）——
        if (strict) {
        ...
        if (strictPlus) {
            // strict+ 额外把其余弱检测也升为 mitigate，全面收紧
            overrides.put("checks.fast-use.enforcement", "mitigate");
            overrides.put("checks.fast-bow.enforcement", "mitigate");
            overrides.put("checks.chest-stealer.enforcement", "mitigate");
            overrides.put("checks.auto-block.enforcement", "mitigate");
        }
