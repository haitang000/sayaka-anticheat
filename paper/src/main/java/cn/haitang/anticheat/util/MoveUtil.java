...
public final class MoveUtil {
     ...
     * 方块所在区块是否已加载。
     *
     * <p>Paper 上对未加载区块调 {@code World#getBlockAt} 会**在主线程强制同步加载区块**。
     * 这些方法跑在每个移动包上，在世界边界、传送后、玩家跑在区块生成前面时，
     * 一次同步加载就是几百毫秒的停顿——而卡顿又会反过来触发各计时检测的误报。
     * 因此所有方块查询前都先判定区块，未加载时一律取「宽松」的那个答案。
     */
    ...
    private static boolean isChunkLoaded(Location loc) {
        ...
        return world != null && isChunkLoaded(world, loc.getBlockX(), loc.getBlockZ());
    }
    ...
    public static boolean hasCollisionBelow(Location loc, double depth) {
        ...
        for (int x = bx0; x <= bx1; x++) {
            for (int z = bz0; z <= bz1; z++) {
                if (!isChunkLoaded(world, x, z)) return true; // 未加载：按有支撑处理
    ...
    public static boolean isClimbing(Player player) {
        ...
        if (!isChunkLoaded(loc)) return true; // 未加载：按可攀爬处理（宽松方向）
    ...
    public static boolean isInWeb(Player player) {
        Location loc = player.getLocation();
        if (!isChunkLoaded(loc)) return true; // 未加载：按在蛛网中处理（宽松方向）
    ...
    public static boolean isNearHoney(World world, int bx, int by, int bz) {
        for (int dx = -1; dx <= 1; dx++) {
            for (int dz = -1; dz <= 1; dz++) {
                // 未加载：按贴着蜂蜜处理（该判定用于豁免悬浮，宽松方向是 true）
                if (!isChunkLoaded(world, bx + dx, bz + dz)) return true;
    ...
    public static boolean touchingLiquid(Player player) {
        ...
        World world = loc.getWorld();
        if (world == null) return false;
        // 未加载：按接触液体处理（该判定用于豁免，宽松方向是 true）
        if (!isChunkLoaded(world, loc.getBlockX(), loc.getBlockZ())) return true;
        Material feet = loc.getBlock().getType();
        Material eye = player.getEyeLocation().getBlock().getType();
