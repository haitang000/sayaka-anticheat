...
- **协议层证据，不信客户端**：基于 PacketEvents 数据包时间线验证攻击、移动时钟、挥臂、击退响应与非法数据包，客户端伪造的 Bukkit 合成事件骗不过检测。
- **32 项检测 + 递进式处置**：覆盖移动、战斗、玩家行为、世界交互、聊天与数据包六大类；每项检测独立累积 VL，按警告 → 拦截 → 踢出 → 临时封禁逐级升级。
- **误判防护优先**：内置鞘翅/攀爬/液体/击退/传送/进服宽限等场景豁免，兼容技能插件位移、区域保护击退削弱等第三方行为；弱启发式检测默认仅警报，基岩版默认走 conservative 策略。
- **免重启热更新**：`/sac update` 下载、校验并隔离热重载，失败自动回滚旧 JAR；Velocity 端"宿主 + 可热替换内核"架构同样支持免重启换载。
- **群组服一体化**：Velocity 组件通过 MariaDB/MySQL 共享白名单、strike、处罚、封禁与申诉记录，内置带申诉入口的 HTTP 管理面板。
- **第三方插件 API**：`sayaka-anticheat-api` 只依赖 Paper API，提供 VL 查询、可取消的违规/处罚事件与自定义豁免判定器，方便其他插件深度集成。
- **低开销**：移动热路径按 tick 节流环境采样，Aim 量子扫描步长优化，数据库连接池复用，高负载下也稳。
