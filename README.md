...
KillAura 除大角度、多目标等显性特征外，还针对"低调配置"（单目标、小视角、正常 CPS、人性化平滑瞄准）提供旋转量化与零反应再瞄准两条包级判定线，并附命中率辅助统计——合成视角改不掉"增量必为灵敏度量子整数倍"与"转正与攻击同刻完成"这两个本质破绽，详见 `config.yml` 的 `quantum` / `acquisition` / `accuracy` 段。

Timer 另含 Blink/掉包器辅助识别：只统计"暂停期间事务 Pong 仍正常返回"的憋包-爆发循环，真实网络抖动与客户端冻结都不会计入，多次循环才产生管理员警报（不计 VL）。NoSlow 与 GroundSpoof 在证据成立时会回弹到最后合法落点（`setback: true`）。BadPackets 同时拦截自击包（攻击自己实体 id）；Scaffold 含不可见面判定（从方块背面放置只能来自伪造包）；Nuker 检测按"合法挖掘级联宽限"排除连锁挖掘插件后默认开启。
...
| `/sac web` | 生成 2 分钟内有效、仅可使用一次的管理后台登录链接 | `anticheat.admin` |
...
Velocity 端同样支持免重启热更新：插件采用"宿主 + 可热替换内核"架构（Velocity 没有运行时卸载插件的 API，`@Plugin` 宿主壳永驻，业务内核可整体换载）。在面板"系统与更新"里下载暂存后点击**热重载应用**，宿主会从新 jar 的子优先类加载器装载新内核，先构造成功再停旧启新；新内核启动失败自动回退到内嵌内核，服务不失联。换载成功后会尽力把新 jar 同步覆盖 `plugins` 中的旧文件（Windows 下文件被锁定时按控制台提示手动替换，否则代理重启会回到旧版本）。

群组模式下，在线管理员可在任一 Paper 后端执行 `/sac web`。Paper 会通过插件消息向 Velocity 请求一次性票据，Velocity 随后返回可点击的管理后台链接。使用域名、HTTPS 或反向代理时，请将 Velocity `config.toml` 的 `web.public-url` 设置为管理员实际访问的完整地址，例如 `https://sac.example.com/`。
