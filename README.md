...
- 🛡️ **协议层证据**：基于 PacketEvents 数据包时间线验证攻击、移动、击退等行为，不信客户端伪造的事件
- 🎯 **32 项检测 + 递进式处置**：六大类检测覆盖，VL 按警告 → 拦截 → 踢出 → 临时封禁逐级升级
- 🛟 **误判防护优先**：内置鞘翅/攀爬/液体/击退/传送/进服宽限等豁免，弱启发式默认仅警报
- ⚡ **免重启热更新**：`/sac update` 隔离热重载，失败自动回滚旧 JAR
- 🌐 **群组服一体化**：Velocity + MariaDB 共享白名单、处罚、封禁与申诉，内置管理面板
- 🔌 **第三方插件 API**：只依赖 Paper API，VL 查询、可取消事件与自定义豁免，方便深度集成
- 💨 **低开销**：热路径节流采样 + 连接池复用，高负载下也稳
